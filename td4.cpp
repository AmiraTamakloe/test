...
shared_ptr<Acteur> ListeFilms::trouverActeur(const string& nomActeur) const
{
	for (const Film* film : enSpan()) {
		for (auto& acteur :film->spanListeActeurs()) {
...
)
{
	...
	film->setAnneePublication(lireUint16(fichier)) ;
	film->recette_ = lireUint16(fichier);
	auto nActeurs = lireUint8(fichier);
	film->acteurs_ = ListeActeurs(nActeurs);

	
	film->setActeurs(ListeActeurs(nActeurs));
	...
	film->acteurs_ = ListeActeurs(film->acteurs_.getCapacite(), film->acteurs_.getNElements());
...
{
	o << "Titre: " << film.getTitre() << "\n";
	o << "Réalisateur: " << film.getRealisateur() << "\n";
	o << "Année :" << film.getAnnee() <<"\n";
	o << "Recette: " << film.getRecette() << "M$ " <<"\n";
	o << "Acteurs:" << "\n";
	
	for (const auto& acteur : film.spanListeActeurs())
...
int main()
{
	...
	//partie 3 
	skylien.acteurs_[0] = listeFilms[1]->getActeurs()[0];
	//partie 4 
	skylien.getActeurs()[0]->nom = "Daniel Wroughton Craig";
	//partie 5
	...
	Film* resultat = listeFilms.trouverFilm([](const Film* film){return film->getRecette() == 955;});
